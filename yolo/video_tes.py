...
with tf.Session() as sess:
    ...
    for i in range(video_frame_cnt):
        ...
        for j in range(len(boxes_)):
            ...
            plot_one_box(img_ori, [x0, y0, x1, y1], label=args.classes[labels_[j]] + ', {:.2f}%'.format(scores_[j] * 100), color=color_table[labels_[j]])
        cv2.putText(img_ori, '{:.2f}ms'.format((end_time - start_time) * 1000) + str(i), (40, 40), 0,
