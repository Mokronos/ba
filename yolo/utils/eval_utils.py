...
def voc_eval(gt_dict, val_preds, classidx, iou_thres=0.5, use_07_metric=False):
    ...
    my_IoU = list()
    class_recs = {}
    npos = 0  #Number of Ground truth positives
    ...
    fp = np.zeros(nd)
    my_IoU = np.zeros(nd)
    for d in range(nd): #I assume d is the detection
        ...
        if BBGT.size > 0:
            ...
            jmax = np.argmax(overlaps) #maps bbox to gt bbox; matched by highest IoU (jaccard max?)
        ...
        if ovmax > iou_thres:
            ...
            if not R['det'][jmax]:
                tp[d] = 1.
                R['det'][jmax] = 1  #Assign the gt box a bbox; if there is already a bbox assigned to the gt bbox, we use the else and it is a false positive
            else:
                fp[d] = 1.  #fp because of 

        else:
            fp[d] = 1.   # fp because of IoU
    ...
    #print(my_IoU[-1])
    # return rec, prec, ap
    return npos, nd, rec, prec, ap, tp[-1], fp[-1], conf_out


def classification_score(gt_dict, val_preds):
    # Jonas: Classification: 
     # Corrects = Boundingbox class is also a gt class AND img_ids are equal.
     #Classification score:   Poistives/All predicted values
    
    #print('pred', val_preds, '\n')
    #print('gt_dict', gt_dict)
    number_of_corrects = 0
    total_gt_classes_in_img = 0
    #incorrects = 0
    for img_id in gt_dict:
        gt_classes_in_image = list()
        #print('img_id', img_id)
        for obj in gt_dict[img_id]:
            if obj[-1] not in gt_classes_in_image:
                gt_classes_in_image.append(obj[-1])
        total_gt_classes_in_img += len(gt_classes_in_image)
        for prediciton in val_preds:
            #print('prediciton', prediciton[-1])
            #print('prediciton', prediciton[:])
            if prediciton[-1] in gt_classes_in_image and prediciton[0] == img_id:
                del_idx = gt_classes_in_image.index(prediciton[-1])
                del gt_classes_in_image[del_idx]   
                number_of_corrects += 1

            #print('gt_dict', gt_dict[img_id])   #gt_dict [[113.88235294117646, 99.2, 121.03529411764706, 118.08, 1]]     gt_dict [[199.1529411764706, 97.6, 209.88235294117646, 120.64, 0], [225.31764705882352, 122.88, 231.71764705882353, 136.0, 1]]
    #print('number_of_corrects', number_of_corrects)
    #print('number of all detections', len(val_preds))
    if len(val_preds) != 0:
        classfic_score = number_of_corrects/total_gt_classes_in_img
        #print('classfication_score', classfic_score)
        return (classfic_score), 0
 
    
    #print('incorrects', incorrects)
    return 0, 0

def IoU_score(gt_dict, val_preds, iou_thres=0.5):
    npos = 0
    class_recs = {}
    for img_id in gt_dict:
        R = [obj for obj in gt_dict[img_id] if obj[-1] == obj[-1]] #this eradicates the selection of the classes and simply uses all classes; Therefore this metric uses all class simultaniuosly
    ...
    # 2. obtain pred results
    pred = [x for x in val_preds if x[-1] == x[-1]]
    img_ids = [x[0] for x in pred]
    confidence = np.array([x[-2] for x in pred])
    BB = np.array([[x[1], x[2], x[3], x[4]] for x in pred])
    ...
    fp = np.zeros(nd)
    bad_IoU = np.zeros(nd)
    good_IoU_but_duplicate = np.zeros(nd)
    #print('class_recs', class_recs)
    #print("type", type(class_recs))
    #class_key = list(class_recs.keys())
    #print(testvar)
    ...
    #print('test', class_recs[img_ids])
    #print("[d]", class_recs[img_ids[d]])
    for d in range(nd): #I assume d is the detection, but it is rahter random. LOL
        # all the gt info in some image
        R = class_recs[img_ids[d]]
        bb = BB[d, :]
        #print('bb', bb)
        ...
        if ovmax > iou_thres:
            ...
            if not R['det'][jmax]:
                ...
                fp[d] = 1.  #fp because of duplicates?
                good_IoU_but_duplicate[d]= 1.

        else:
            fp[d] = 1.   # fp because of IoU
            bad_IoU[d] = 1.   # fp because of IoU
        #print('R', R)
    #print('tp', tp)
    fp = np.cumsum(fp)
    tp = np.cumsum(tp)
    bad_IoU = np.cumsum(bad_IoU)
    good_IoU_but_duplicate = np.cumsum(good_IoU_but_duplicate)
    print('number_of_detections', len(tp))
    print('iou_tp_over_all_classes', tp[-1])
    print('bad_IoU', bad_IoU[-1])
    bad_IoU = bad_IoU[-1]
    good_IoU_but_duplicate = good_IoU_but_duplicate[-1]

    return bad_IoU, good_IoU_but_duplicate, tp[-1], nd