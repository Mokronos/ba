...
def random_color_distort(img, brightness_delta=32, hue_vari=18, sat_vari=0.5, val_vari=0.5):
    ...
    def random_hue(img_hsv, hue_vari, p=0.0001):
        if np.random.uniform(0, 1) > p:
            hue_delta = np.random.randint(-hue_vari, hue_vari)
            img_hsv[:, :, 0] = (img_hsv[:, :, 0] + hue_delta) % 180
        return img_hsv

    def random_saturation(img_hsv, sat_vari, p=0.25):
        if np.random.uniform(0, 1) > p:
            sat_mult = 1 + np.random.uniform(-sat_vari, sat_vari)
            img_hsv[:, :, 1] *= sat_mult
        return img_hsv

    def random_value(img_hsv, val_vari, p=0.0001):
        if np.random.uniform(0, 1) > p:
            val_mult = 1 + np.random.uniform(-val_vari, val_vari)
            img_hsv[:, :, 2] *= val_mult
        return img_hsv

    def random_brightness(img, brightness_delta, p=0.25):
        if np.random.uniform(0, 1) > p:
...
def random_color_distort_eval(img, brightness_delta=32, hue_vari=18, sat_vari=0.5, val_vari=0.5):
    '''
    randomly distort image color. Adjust brightness, hue, saturation, value.
    param:
        img: a BGR uint8 format OpenCV image. HWC format.
    ...
    def random_hue(img_hsv, hue_vari, p=0.00):
        if np.random.uniform(0, 1) > p:
            hue_delta = np.random.randint(-hue_vari, hue_vari)
            img_hsv[:, :, 0] = (img_hsv[:, :, 0] + hue_delta) % 180
        return img_hsv

    def random_saturation(img_hsv, sat_vari, p=1):
        #if np.random.uniform(0, 1) > p:
        sat_mult = 1 + 0.5 #np.random.uniform(-sat_vari, sat_vari)
        img_hsv[:, :, 1] *= sat_mult
        return img_hsv

    def random_value(img_hsv, val_vari, p=0.000):
        if np.random.uniform(0, 1) > p:
            val_mult = 1 + np.random.uniform(-val_vari, val_vari)
            img_hsv[:, :, 2] *= val_mult
        return img_hsv

    def random_brightness(img, brightness_delta, p=0):
        if np.random.uniform(0, 1) > p:
            img = img.astype(np.float32)
            brightness_delta = int(np.random.uniform(-brightness_delta, brightness_delta))
            img = img + brightness_delta
...
def letterbox_resize(img, new_width, new_height, interp=0):
    ...
    #status = cv2.imwrite('/home/hendljonas/Downloads/letterbox_resize+{}+dw{}.jpg'.format(resize_ratio, dw),image_padded) #Save Image; crashes cudnn quickly
 
    #print("Image written to file-system : ",status) #check if saved
...
def random_expand(img, bbox, max_ratio=4, fill=0.5, keep_ratio=True):   #fills with black values. fill = 0, problematic if resize with bbx is grey cause it creats lines. These lines should be detected by the featrue detector. Changed to grey.
    '''
